...
To connect to Metamask wallet, call `Web3Connect()` method from `Assets/Plugins/Web3Unity/Scripts/Scenes/WebLogin.cs`.

After successful logging in the user’s address will be available at `PlayerPrefs.GetString("Account")`.

See `Assets/Plugins/Web3Unity/Scenes/WebLogin.unity` scene as an example.

## Authorization

To authorize and connect to the GTS API create a new `Connection` class from `GravityTS.Utils` namespace.

```GTSConnection = new Connection(_apiUrl, _account);```

where<br/>
`_apiUrl` is a link to GTS API,<br/>
